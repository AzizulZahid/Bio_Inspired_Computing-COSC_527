...
class CASimulator():
    ...
    def simulate(self):
        ...
        for experiment in range(self.num_experiments):
            ...
            for z in range(len(self.rule_table)):
                ...
                H = self.calculate_H()
                # density = self.compute_non_zero_density()
                density = self.SD_to_mean()
                # print(density)
                ...
                f.write(str(z) + "," + entry_zeroed + ",," + str(lam) + "," + str(lam_T) + "," + str(H) + "," + str(H_T) + "," + str(density) + ",,\n") 
    ...
    # new portion
    def compute_non_zero_density(self):
        non_zero_count = sum(1 for elem in self.rule_table if elem != 0)
        total_elements = len(self.rule_table)
        return non_zero_count / total_elements         
        
        # new portion
    def SD_to_mean(self):
        SD = np.std(self.rule_table)
        Mean = np.mean(self.rule_table)
        return (SD+0.1)/(Mean+.03)
